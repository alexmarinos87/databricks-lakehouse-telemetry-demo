...
capabilities were being inspected. PR #131 restored the accepted tree, but the
existing contract suite did not classify root-level scratch markers as invalid
repository content.
...
- Repeating the cleanup completed by PR #131.
...
- Run exact-head `CI / validate` and artifact compatibility on the rebuilt PR.
