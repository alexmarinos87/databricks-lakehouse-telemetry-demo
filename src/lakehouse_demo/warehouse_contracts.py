...
def _assignment_conflicts(rows: tuple[Row, ...]) -> list[WarehouseContractFinding]:
    machines: dict[str, tuple[object, dict[tuple[str, ...], KeyValues]]] = {}
    for row in rows:
        machine_id = row.get("machine_id")
        token = _stable_value(machine_id)
        _, assignments = machines.setdefault(token, (machine_id, {}))
    ...
    for machine_id, assignments_by_token in machines.values():
